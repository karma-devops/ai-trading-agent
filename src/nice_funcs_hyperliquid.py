...
# ============================================================================
# Use HyperLiquid leverage from config as the single source of truth
from src.config import HYPERLIQUID_LEVERAGE
...
def get_position(symbol_or_address, account=None):
    """
    HyperLiquid-only get_position.
    Queries the 'ACCOUNT_ADDRESS' from env for positions,
    falling back to the signing wallet address if needed.
    """
    try:
    ...
    # Handle multiple open subpositions individually
...
def ai_entry(symbol, amount, max_chunk_size=None, leverage=None, account=None):
...
def open_short(token, amount, slippage=None, leverage=None, account=None):
